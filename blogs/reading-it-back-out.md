...
Go back to why I moved from behaviour to weights. In the last post the identity was inferred from what the model chose to say, and the values only showed up in flashes. Here it shows up before the model says anything, as a direction you can read, steer, ablate and edit. And at least one imprinted arm defends against being steered elsewhere. That is a stronger claim than "the personas are vivid", and it is the mechanistic half of the same story: behaviour in, an identity inferred, and now the identity located as a real, movable thing in the residual stream.

It is still small and still screening-grade, and I would rather be plain about the size of it than round it up. `4B` is a small model, the refits are single-seed, the confirmatory effect is `7e-4`, my confidence intervals collapse the three seeds before bootstrapping so they understate seed variance, and the shared-subspace result means I cannot yet claim each religion owns a private direction. The steering, ablation and interference are single-seed too, one dose and, for the interference, one anchor, so the causal side is more of a first look than the correlational side. Some planned pieces, a band sweep and a single-layer localisation, I never ran. The intervention machinery, the concept vectors and the swap and ablate operators, is written up properly in the repo rather than padded into this post. The next moves are the boring, correct ones: more seeds so the validity control has power on the losers too, `8B` and up where the ground is firmer, and pulling the shared religiosity direction apart from the per-religion parts instead of reading them tangled. This is a step toward the same thing the parent post was reaching for, whether a value distribution you have chosen can be loaded from behaviour alone, and now whether you can find it, name it, and move it once it is in there.
